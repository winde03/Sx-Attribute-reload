...
public abstract class SubCondition {
     ...
     * @return int 没有则为-1
     */
    public static int getItemLevelnew(ItemStack item) {
        if (item != null && item.hasItemMeta() && item.getItemMeta().hasLore()) {
            return item.getItemMeta().getLore().stream().filter(lore -> lore.contains(SkillAPI.getSettings().getLoreLevelText())).findFirst().map(lore -> Integer.valueOf(getNumber(lore).replace(".", ""))).orElse(-1);
        }
        return -1;
    }
    /**
